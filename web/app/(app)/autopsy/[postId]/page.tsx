...
export default function AutopsyPage() {
  ...
  const view = useMemo(() => {
    ...
    const postMetrics = readJson<MetricPoint[]>(STORE_KEYS.metrics, []).filter((m) => m.postId === params.postId);
  ...
  return <><h1>Post Autopsy</h1><section className="card"><p>Post ID: {params.postId}</p><p>Predicted score: {view.predicted}/100</p><p>Expected impressions: {view.expectedImpressions}</p><p>Actual impressions: {view.actualImpressions}</p><p>Delta: {view.delta}</p></section></>;
}
